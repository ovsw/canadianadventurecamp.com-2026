...
2. Upload the same files to the Raster library `latest`. This is the
   permanent ingestion library. Library size does not matter: the sync skips
   assets that are already in sync and only lists the new ones. If you ever
   use a different library, grant the API key access to it in Raster first.
