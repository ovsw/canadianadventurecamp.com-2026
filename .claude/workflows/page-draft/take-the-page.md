...
One agent. Input: a target, or nothing. About ten commands; every one you
need is written below with its flags. Basecamp ids are in `page-workflow.md`
"Basecamp".
...
target was an issue number, return that number: the script skips research
and planning. Otherwise search once,
